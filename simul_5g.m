function [Pt,Pt_central_cell,SINR_user,reject] = simul_5g(input_matrix,H_final)
...
max_users_per_BS = input_matrix(7);
PRB_per_MS = input_matrix(8);
...
subcarriers_of_sectors = zeros(total_sectors,N_subcarriers);
subcarriers_of_users = zeros(1,PRB_per_MS);
...
Pt = zeros(size(position,1),1);
Pt_central_cell = 0;

j  = 0;
BS_set = 1:1:number_of_cells;

while(flag==0)
   candidate_BS = randsrc(1,1,BS_set);
   users_BS_previous = users_BS;
   BS_set_previous = BS_set;
   x_min = position(candidate_BS,1) - R; 
   x_max = position(candidate_BS,1) + R;                            
   y_min = position(candidate_BS,2) - R*(sqrt(3)/2);               
   y_max = position(candidate_BS,2) + R*(sqrt(3)/2);
   [x_pos,y_pos,z_pos,PL,base_station,sector,angle]  = users_distribute(fc,x_min,x_max,y_min,y_max,Rc,position,PL_max,h_UT);
   while(base_station~=candidate_BS)
       [x_pos,y_pos,z_pos,PL,base_station,sector,angle]  = users_distribute(fc,x_min,x_max,y_min,y_max,Rc,position,PL_max,h_UT);
   end
   users_BS(1,base_station) = users_BS(1,base_station) + 1;
   if(users_BS(1,base_station)==max_users_per_BS)
       BS_set(find(BS_set==base_station)) = []; 
   end
   j = j + 1;
   reject = 0;
   ...
   n = sector_user(sum(users),1);
%    for PRB_index = 1:1:N_subcarriers
% %         h = channel_3gpp_5G(Mt,Mr,fc,h_UT,d2D);
%         h = H_final;
% %         H(sum(users),n,1:Mr,1:Mt,PRB_index) = h(1:Mr,1:Mt);
%    end
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   final_temp_index = zeros(1,PRB_per_MS);
   final_h = zeros(Mr,Mt,PRB_per_MS);
   subcarriers_of_sectors_previous = subcarriers_of_sectors;
   temp_P = zeros(1,N_subcarriers);
   for sub_index = 1:1:PRB_per_MS
        pool_of_subcarriers = find(subcarriers_of_sectors(sector_user(sum(users),1),1:N_subcarriers)==0); 
        temp_gain = 0;
        if(rrm_type==1)
            for channel_index = 1:1:length(pool_of_subcarriers)
                 current_h = zeros(Mr,Mt);
                 %current_h(1:Mr,1:Mt) = channel_3gpp_5G(Mt,Mr,fc,h_UT,d2D);
                 current_h(1:Mr,1:Mt) = H_final;
                 ...
                 if((temp_t'*(current_h'*current_h)*temp_t)>temp_gain)
                     ...
                     final_h(1:Mr,1:Mt,sub_index) = current_h;
                     P_users(sum(users),temp_index) = temp_gain*(SNR_req*No*10^(total_loss_user_own_sector(1,sum(users))/10));
                 end
            end
            if(base_station==1)
                Pt_central_cell = Pt_central_cell + real(sum(P_users(sum(users),:)));
            end
            subcarriers_of_sectors(sector_user(sum(users),1),temp_index) = 1;
        else
   ...
   if(reject>0)
       ...
       users(1,3*(base_station-1) + sector) = users(1,3*(base_station-1) + sector) - 1;  
       subcarriers_of_sectors  = subcarriers_of_sectors_previous;
       users_BS = users_BS_previous;
       BS_set = BS_set_previous;
   else
       H(sum(users),n,1:Mr,1:Mt,1:PRB_per_MS) = final_h(1:Mr,1:Mt,1:PRB_per_MS);
   ...
   flag = (sum(users_BS)==number_of_cells*max_users_per_BS);
end

Pt = real(sum(sum(P_users)));
index_user = 1;
...
sub_index = 1;
    ...
    h = zeros(Mr,Mt);
    h(1:Mr,1:Mt) = H(index_user,sector_user(index_user,1),1:Mr,1:Mt,find(order_of_subcarriers_of_users(index_user,:)==temp_index));
    ...
    for index_MS = 1:1:length(co_channel_MSs)
        if(co_channel_MSs(index_MS)~=index_user)
           ...
           h_int = zeros(Mr,Mt);
           %h_int(1:Mr,1:Mt) = channel_3gpp_5G(Mt,Mr,fc,h_UT,d2D);
           h_int(1:Mr,1:Mt) = H_final;
